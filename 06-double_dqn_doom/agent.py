...
class DQNAgent():
    def __init__(self, gamma, epsilon, lr, n_actions, input_dims, mem_size, batch_size=32,
                 eps_min=0.1, eps_dec=1e-5, tau=1000, env_name='Doom', chkpt_dir='models/'):
        self.action_to_game = [list(a) for a in itertools.product([0, 1], repeat=3)]
    ...
    def learn(self):
        ...
        with torch.no_grad():
            q_next = self.q_next.forward(states_).detach().max(dim=1)[0]  # predict the max value of the
...
# def learn(self):
    ...
    #     q_pred = self.q_eval.forward(states)  # select the values only for actions the agent have taken actions 0 or 1
    #     q_target = q_pred.detach().clone()
    #     q_next = self.q_eval.forward(states_).detach().max(dim=1)[0]  # predict the max value of the
    #     # with torch.no_grad():
    #     #     q_next = self.q_next.forward(states_).detach().max(dim=1)[0]  # predict the max value of the
    #     q_next[dones] = 0.0  # for terminal states, there's no other state ahead, so reward = 0.
    #     q_target[indices, actions] = rewards + self.gamma * q_next
